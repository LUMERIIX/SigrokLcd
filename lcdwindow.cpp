...
wxLCDWindow::wxLCDWindow( wxWindow *parent, wxPoint pos, wxSize size ) : wxWindow( parent, -1, pos, size, wxSUNKEN_BORDER )
{
	mSegmentLen = 40;
	mSegmentWidth = 10;
	mSpace = 5;
...
//    case 7:
//		points[0].x = x + sw;
//		points[0].y = y + 2*sl - sw;
//		points[1].x = x + sw;
//		points[1].y = y + 2*sl - 3*sw/2;
//		points[2].x = x + sl - 3*sw/2;
//		points[2].y = y + sl + sw/2;
//		points[3].x = x + sl - sw;
//		points[3].y = y + sl + sw/2;
//		points[4].x = x + sl - sw;
//		points[4].y = y + sl + sw;
//		points[5].x = x + 3*sw/2;
//		points[5].y = y + 2*sl - sw;
//        break;
	default:
		break;
	}

    dc->DrawPolygon( segment < 6 ? 4 : 6, points );
}

void wxLCDWindow::DrawDot( wxDC *dc, int digit, bool state )
{
    int sl = mSegmentLen;
	int sw = mSegmentWidth;
...
int wxLCDWindow::GetDigitWidth()
{
	return mSegmentLen + mSegmentWidth + mSpace;
}


int wxLCDWindow::GetDigitHeight()
{
	return ( 2 * mSegmentLen ) + ( 2 * mSpace );
}


int wxLCDWindow::GetBitmapWidth()
{
	return ( mNumberDigits * GetDigitWidth() ) + mSpace;
}


int wxLCDWindow::GetBitmapHeight()
{
...
wxString wxLCDWindow::GetValue()
{
	return mValue;
}


int wxLCDWindow::GetNumberDigits()
{
...
wxColour wxLCDWindow::GetLightColour()
{
	return mLightColour;
}


wxColour wxLCDWindow::GetGrayColour()
{
	return mGrayColour;
}
